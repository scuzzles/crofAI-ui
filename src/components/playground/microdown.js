// credit to https://github.com/commit-intl/micro-down
...
var inlineBlock = (text, dontInline) => {
  var temp = [],
    injectInlineBlock = (text) =>
      text.replace(/\\(\d+)/g, (match, code) => injectInlineBlock(temp[Number.parseInt(code) - 1]));

  text = (text || "")
    .trim()
    // inline code block
    .replace(/`([^`]*)`/g, (match, text) => "\\" + temp.push(tag("code", encode(text))))
...
var p = (text, rules, parse, options) => {
  var i = 0,
    f;
...
export var parse = (text, options) => {
  // clean input
  text = text.replace(/[\r\v\b\f]/g, "").replace(/\\./g, (match) => `&#${match.charCodeAt(1)};`);
